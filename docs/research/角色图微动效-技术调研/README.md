...
每张图的增量成本是 **一个约 20KB 的权重图 + 约 300ms 的离线计算**，运行时开销接近于零（无 GPU 的纯软件渲染下仍有 100fps）。六种画风实测全部通过，包括三种二次元。

**但样本只有 8 张，移动端一个数字都没有。** 现在的结论是"路线成立"，不是"可以上线"。

---

## 1. 为什么做

角色图作为聊天背景常驻在屏幕上。完全静止的图，用久了就"死"了——用户会意识到自己在看一张图片，而不是在跟一个人说话。

微动效的作用是**提供沉浸感，不是提供视觉焦点**。它应该是那种"你说不清哪里在动，但就是感觉她在那儿"的东西。
...
| # | 路线 | 验证程度 | 结论 |
| --- | --- | --- | --- |
| A | 生成式视频（即梦 Seedance） | 跑过 API | **排除** · 成本不成比例 |
...
**排除理由不是效果不好，是成本不成比例。** 为了 2px 的头发飘动去调 5 秒视频扩散模型：每张烧积分、等几分钟、结果抽卡不可控，产物 500KB–1MB（vs 现方案 20KB）。1000 个角色就是持续烧额度。

> **诚实记录**：四次提交全部被平台并发限制挡回，我们**没有拿到成片**。排除决策是基于成本结构，不是基于看到的效果。如果哪天需要"头发大幅飘动"这种强动效，这条路应该重新评估。
...
> **请不要把原型的表现当成结论。** 整个验证只基于 6 张角色图 + 2 张风景图，且全部是手边现成素材，不是抽样。

- **画风覆盖不全** — 已测：半写实 / 油画 / 3D 渲染 / 三种二次元。**未测**：平涂线稿、漫画分镜、厚涂、像素风、水墨。
- **构图覆盖不全** — 未测：大头特写、多人（>2）、严重遮挡、角色占比极小。
- **移动端完全没测** — 帧率、耗电、发热、低端机降级，**一个数字都没有**。这是唯一真金白银的运行时成本。
- **景物只验证了检测，没验证运动** — 找云 / 海 / 草的位置是准的，但**云该怎么飘、浪该怎么走，一行代码都还没写**。现有渲染内核只有"发根锚定摆动"一种运动模型，对云（无锚定、周期长 5–10 倍）和海浪（定向行波）都不适用。
...
算法同学 ×2 已到位。建议按「哪里动」/「怎么动」切分，中间用固定的 `weight.png + meta.json` 契约对接，**两边可以完全并行**。
...
- 一台中端安卓机用于功耗实测
- 出图管线里把 prompt 的景物名词抽成结构化字段——**这是我们独有的信息优势**：外面做同类产品的团队不知道图里有什么，我们知道，直接喂给文本提示分割命中率高得多
...
> 原型和需求说明目前在本地 `桌面/滴滴/17.互动内容产品/微动效原型/`，尚未入库。
> 如需在线预览版，找 Serac 要 Artifact 链接（默认私有，需手动分享）。

---

*本文档记录截至 2026-08-12 的调研状态。所有数字均为实测，未验证的部分已在第 8 节明确标注。*
